...
| 2026-08-07 ~19:0x | **ROUND-TRIP PROTOCOL deployed (the hidden protocol from the AdiiX article; commit to be noted).** The 98%-win-rate mechanism was never the entry — it's the EXIT: the article's bot "exits at this point for an immediate profit from the repricing" 2-3s after entering the lagging side. That is a bet that the market CORRECTS (near-certain), not a bet on the final outcome (a coin flip — our calibration: ~52%). We previously held every directional position to settlement, silently converting good lag entries into EV-negative outcome bets. Changes: (1) NEW `REPRICE` exit in `_check_early_exits` — exit when the held token has gained ≥ `REPRICE_EXIT_GAIN_PCT` (0.07, clears the ~4.1% round-trip fee) within `REPRICE_EXIT_MAX_HOLD_S` (240s); book reads now prefer the zero-latency WS cache so exits are as fast as entries; (2) `EDGE_THRESHOLD_PCT` 0.05 → 0.04 (the article's "typically 3-5%" divergence band); (3) `MIN_CONFIDENCE` 0.85 → 0.75 (0.85 demanded ≥ $2100 book depth — nearly never true on live books, silently throttling the fast path to 0 trades/day; 0.75 keeps the stale-data protection while letting ~$600+ books trade). Sum-to-one legs remain exempt from REPRICE. Tests: **380 passing.** |
