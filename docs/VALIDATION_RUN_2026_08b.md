...
| 2026-08-13 | **Second risk-free leg added: CROSS-WINDOW (same-endTime 5m/15m) arbitrage — sourced from public Polymarket-bot research, verified mathematically before building, NOT copied.** User request: "i want you to use whats usefull on other arbitrage bots in the web". The research was verified repo-by-repo (radioman = repo-squatting scam, avoid; gamma-trade-lab / zkOSAI / Trum3it / machenxi = DO NOT EXIST on GitHub — hallucinated by the external review; ImMike/polymarket-arbitrage = real, cross-platform detection only). The ONE mathematically valid, buildable idea found: two windows on the SAME asset with the SAME endTime resolve against the SAME final price but have DIFFERENT beat prices (each window's price at its own open). Buying UP on the lower-beat window + DOWN on the higher-beat window pays **≥ $1 for every possible outcome** (exactly $1 in the outer bands, **$2 in the middle band**) — the same arithmetic guarantee as sum-to-one, exploited across windows instead of across YES/NO tokens. New pieces: `engine/cross_window.py` (payoff math + pairing + detection, fully unit-tested on all three payoff bands), `CROSS_WINDOW_*` settings (enabled / min edge 1% / max position 10% / min beat gap 0.05% / max reference-capture delay 10s), `PaperBroker.place_cross_window_order` (mirrors sum-to-one: equal-share sizing, both legs concurrently, pre-quote walk refusal, post-fill edge re-validation with reverse-vs-hold), `main.py._scan_cross_window_universe` (groups `_known_markets` by asset+endTime, one entry per cycle, near-miss diagnostics surfaced as `cross_window_scan`), and the early-exit guard extended to skip `cross_window` legs (a directional exit on one leg breaks the hedge — the exact 2026-08-07 sum-to-one mistake, prevented from the start this time). **Critical honesty about the beat source:** Gamma does NOT serve beat prices, so pairing requires each window's reference to have been captured within `CROSS_WINDOW_MAX_REF_CAPTURE_DELAY_S` (10s) of its open and the beats to differ by ≥ 0.05% — an ordering mistake would flip the guaranteed arb into a directional bet, so the scan FAILS CLOSED on any untrusted reference (tested). The directional leg and all frozen thresholds are untouched; this is an additive risk-free leg, same class as sum-to-one. 496 tests pass (18 new). |

---

| 2026-08-15 | **CORRECTNESS FIX (no thresholds touched): a risk halt (daily halt OR kill switch) was strangling EXIT management, not just entries.** Verified against the live record, not guessed: all 36 trades since 08-13 are directional `latency_arb` — 18W/18L, **−$354.71, PF 0.27** — and the four SETTLED-at-zero losses (−$70, −$58, −$69, −$8) all share one signature: never-green positions (MFE < 0) that should have been NO_PROGRESS-cut at 120s, but rode 4+ hours to settlement at $0. Root cause in `main.py._trading_cycle`: `if not self.risk.is_trading_allowed(): return` came BEFORE `_check_early_exits`, so when the kill switch tripped (drawdown crossed the 40% threshold during the 19:37-19:38 loss cluster on 08-13) the ENTIRE exit path stopped — REPRICE, GAP_EXPIRED, NO_PROGRESS, EDGE_REVERSAL all dead — and the bot sat parked for ~2 days, evaluating 0 signals (the risk-free scan dicts were empty for the same reason: the scans live after the same gate). The `/pause` path already keeps managing open positions; the risk-halt path must do no less. **Fix:** the risk-halt branch now runs `_check_early_exits` before returning — a halt stops NEW ENTRIES, never the protection of existing positions (settlement already runs in its own independent loop). **Second fix, same root cause:** the kill switch had NO resume path except DB surgery, so the parked bot was unrecoverable by the operator. Added a `/reset` Telegram command (audited: logged with an operator note, re-seeds the drawdown baseline, chat-gated to the configured chat — deliberately NOT a button, so a fat-finger tap can never silently clear a drawdown kill switch). This is infrastructure/correctness, not a strategy change: the directional leg stays frozen as-is (its 36-trade record is the forward sample the freeze is protecting), and the risk-free legs were never firing partly BECAUSE the halt parked everything behind the same gate. 499 tests pass (4 new: risk-halt still runs NO_PROGRESS exits through the full trading cycle; /reset reaches controls; /reset is chat-gated). |
